...
namespace {
struct ConvertRiseToStandardPass
    : public ModulePass<ConvertRiseToStandardPass> {
...
PatternMatchResult
LiteralLowering::matchAndRewrite(LiteralOp literalOp,
                                 PatternRewriter &rewriter) const {
  Location loc = literalOp.getLoc();
  LiteralAttr literalAttr = literalOp.getAttrOfType<LiteralAttr>("literal");
  if (!literalAttr) {
    printf("we got a nullptr here, captain! \n");
  }
  ...
  case RiseTypeKind::RISE_INT: {
    ...
    rewriter.create<ConstantIntOp>(
        loc, value,
        IntegerType::get(32, rewriter.getContext())); // why is this getting
                                                      // such a strange SSA id?
    break;
  }
  case RiseTypeKind::RISE_FLOAT: {
    APFloat value = APFloat(std::stof(literalAttr.getValue()));
    rewriter.create<ConstantFloatOp>(loc, value,
...
PatternMatchResult
LambdaLowering::matchAndRewrite(LambdaOp lambdaOp,
                                PatternRewriter &rewriter) const {
  MLIRContext *context = rewriter.getContext();
  Location loc = lambdaOp.getLoc();

  FunctionType funType =
      FunctionType::get(lambdaOp.getType().cast<FunType>().getInput(),
                        lambdaOp.getType().cast<FunType>().getInput(), context);
  FuncOp fun = rewriter.create<FuncOp>(loc, "testFun", funType,
                                       ArrayRef<NamedAttribute>{});
  Block *funBody = fun.addEntryBlock();

  // Adding the region of the lambdaOp to the FuncOp. Enclosed ops are handled
  // separately
  rewriter.inlineRegionBefore(lambdaOp.region(), *funBody->getParent(),
                              funBody->getParent()->end());
...
PatternMatchResult
ReturnLowering::matchAndRewrite(mlir::rise::ReturnOp returnOp,
                                PatternRewriter &rewriter) const {
  MLIRContext *context = rewriter.getContext();
...
PatternMatchResult
ApplyLowering::matchAndRewrite(ApplyOp applyOp,
                               PatternRewriter &rewriter) const {
  MLIRContext *context = rewriter.getContext();
...
static PassRegistration<ConvertRiseToStandardPass> pass(
    "convert-rise-to-standard",
