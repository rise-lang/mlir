// RUN: mlir-opt %s -convert-rise-to-imperative -convert-linalg-to-loops -convert-loop-to-std -convert-std-to-llvm | mlir-cpu-runner -e mapMapId -entry-point-result=void -shared-libs=%linalg_test_lib_dir/libmlir_runner_utils%shlibext  | FileCheck %s --check-prefix=3D_MAP_ADD

func @print_memref_f32(memref<*xf32>)
func @rise_fun(memref<4x4x4xf32>)
func @mapMapId() {
    ...
    rise.fun "rise_fun" (%outArg:memref<4x4x4xf32>) {
        ...
        %map1 = rise.mapSeq #rise.nat<4> #rise.array<4, !rise.array<4, !rise.float>> #rise.array<4, !rise.array<4, !rise.float>>
        %mapInnerLambda_1 = rise.lambda (%arraySlice_1) : !rise.fun<data<array<4, array<4, float>>> -> data<array<4, array<4, float>>>> {
            %map2 = rise.mapSeq #rise.nat<4> #rise.array<4, !rise.float> #rise.array<4, !rise.float>
            %mapInnerLambda_2 = rise.lambda (%arraySlice_2) : !rise.fun<data<array<4, float>> -> data<array<4, float>>> {
                %map3 = rise.mapSeq #rise.nat<4> #rise.float #rise.float
    ...
    call @rise_fun(%outputArray) : (memref<4x4x4xf32>) -> ()
