// RUN: mlir-opt %s -convert-rise-to-imperative -convert-linalg-to-loops -convert-scf-to-std -convert-std-to-llvm | mlir-cpu-runner -e mapMapId -entry-point-result=void -shared-libs=%linalg_test_lib_dir/libmlir_runner_utils%shlibext  | FileCheck %s --check-prefix=4D_MAP_ADD --dump-input=fail
...
func @mapMapId() {
    ...
    %outputArray = memref.alloc() : memref<4x4x4x4xf32>
    
    %inputArray = memref.alloc() : memref<4x4x4x4xf32>
    %cst = constant 5.0 : f32
    linalg.fill(%cst, %inputArray) : f32, memref<4x4x4x4xf32>
        
    call @rise_fun(%outputArray, %inputArray) : (memref<4x4x4x4xf32>, memref<4x4x4x4xf32>) -> ()

    %print_me = memref.cast %outputArray : memref<4x4x4x4xf32> to memref<*xf32>
